...
- [09 — Ingest is faithful, enrichment is validated](09-faithful-ingest-validated-enrichment.md) — placeholder values ("unknown", "none") are stripped from model output and left alone in source data; the index counts them so the call stays observed.
- [10 — Address is two-tier](10-address-two-tier.md) — street-level `addresses[]` is hold-back and reaches no prompt, export, or MCP result; a coarse `location` travels wherever `employer` does. Enforced by a field allow-list plus a leak test, not a convention.
- [11 — Loud stall over silent recovery](11-loud-stall-over-silent-recovery.md) — writes to the non-regenerable overlays serialize behind a lock that displaces a holder only on proof of death (a known, unclosed race can still admit a second writer without it), and stop with a self-describing error rather than recovering automatically. An age-based lock steal was built, measured to trade a loud failure for a silent one, and removed.
