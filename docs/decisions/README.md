...
- [10 — Address is two-tier](10-address-two-tier.md) — street-level `addresses[]` is hold-back and reaches no prompt, export, or MCP result; a coarse `location` travels wherever `employer` does. Enforced by a denylist plus a leak test, not a convention.
