...
**Battery:** naive (Haiku), adversarial (Sonnet), hypercritical (Sonnet), test (Sonnet) — two independent passes per reviewer (8 passes total).
...
**Model note:** on an Opus leg, so `test` ran on Sonnet per the model-fallback ladder (test→Sonnet, not Opus).
...
- **TOCTOU on concurrent enrichment runs → duplicate quota spend + batch-file overwrite** `[moderate]` — `enrich-batch.mjs:51-93,217,241-247`. Two runs (manual+cron) read the same "not yet attempted" snapshot and pay for the same candidates; batch filenames truncated to the second (`:242`) can collide and silently overwrite. This is the exact bug a sibling private instance already patched with a reservation-sharding flag. **Fix:** lockfile guard + uuid/pid batch-filename suffix. (adversarial.)
